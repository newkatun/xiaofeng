...
class OrderlistAction extends CommonAction{
	...
	public function reTotal($strPost){
		$returnArray=$this->getCountNumber($this->dataTable,$strPost);
		return $returnArray;
	}
	...
	public function saveData($dataArray,$dataType){
		...
		if($dataType['options']=='updateTable'){
			$strArray=empty($dataType['where'])?"autoid=".$dataType['autoid']:$dataType['where'];
	...
	public function checkData($strPost,$dataType){
		$flag=true;
		$strData['order_id'] =CharsCheck($strPost['order_id']);
		$strData['order_uname'] =CharsCheck($strPost['order_uname']);
		$strData['order_uid'] =intval($strPost['order_uid']);
		if(isset($strPost['order_text'])) $strData['order_text'] =CharsCheck($strPost['order_text']);
		
		if(empty($strData['order_id'])){
			$flag=false;
		}
		if(empty($strData['order_uname'])){
			$flag=false;
		}
