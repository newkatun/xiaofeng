 ...
 */	
	$.fn.ChangeNum=function(options){
		...
		return this.blur(function(){
			...
			var pordpoints=$('span.pordpoints'+prodcartid).text();
			var pointsint=parseInt($('span#points_'+prodcartid).attr('pointsvalue'));
			if(checknumber($objview.val())){
				...
				$('span#points_'+prodcartid).text(pointsint*$objview.val());
				var ttime=new Date();
				var t=ttime.getMilliseconds();
				$.get('ajaxfile/get_prodshow.php?action=upnumprod&prodnum='+$objview.val()+'&cartid='+prodcartid+'&t='+t,function(data){
						CheckLoginStatus(data);
						var strarray=data.split(",");
						$('#cart_basket').html(strarray[0]);
						$('#subtotal_'+prodcartid).text(FormartFloat(parseFloat(prodprice)*$objview.val()));
						$('#subtotalpoints_'+prodcartid).text(parseFloat(pordpoints)*$objview.val());
						$('span.total_price_span').text(strarray[1]);	
						$('span.total_sendpoint_span').text(strarray[2]);	
				})
			}else{
	...
	$.fn.ChangeNumBtn=function(options){
		...
		return this.click(function(){
			...
			var prodpoints=$('span.pordpoints'+orderprodid).text();
			var pointsint=parseInt($('span#points_'+orderprodid).attr('pointsvalue'));
			if(methodtype=='add'){
			...
			if(ordernum>0){
				...
				$('span#points_'+orderprodid).text(pointsint*ordernum);
				var ttime=new Date();
				var t=ttime.getMilliseconds();
				$.get('ajaxfile/get_prodshow.php?action=upnumprod&prodnum='+ordernum+'&cartid='+orderprodid+'&t='+t,function(data){
					CheckLoginStatus(data);
					var strarray=data.split(",");
					$('#cart_basket').html(strarray[0]);
					$('#subtotal_'+orderprodid).text(FormartFloat(parseFloat(prodprice)*ordernum));
					
					$('#subtotalpoints_'+orderprodid).text(parseFloat(prodpoints)*ordernum);
					$('span.total_price_span').text(strarray[1]);	
					$('span.total_sendpoint_span').text(strarray[2]);	
	...
	$.fn.DeleteOrder=function(options){
		...
		return this.click(function(){
			...
			if(confirm('Do you realy remove this product?')){
				var prodcartid=$objview.parent().parent().attr('data-cartid');
				$.get('ajaxfile/get_prodshow.php?action=delprod&cartid='+prodcartid,function(data){
					CheckLoginStatus(data);
					var strarray=data.split(",");
					$('#cart_basket').html(strarray[0]);
