<?php
	...
	function LocationUrl($strurl=""){
		if(empty($strurl)){
			...
			echo "<script type='text/javascript'>window.location.href='http://".$_SERVER['HTTP_HOST'].$strurl."';</script>";
		}
	}
	...
	//调整图像地址
	function ChangeImgShow($strImgUrl,$strType='big'){
		if(!empty($strImgUrl)){
			if($strType=='big'){
				return str_replace('/smallimg/','/image/',$strImgUrl);
			}else{
				return str_replace('/image/','/smallimg/',$strImgUrl);
			}
		}
	}
	 ...
	 *创建随机字符
	 */
	function GetRandWords($strnum){
		$strtext="abcdefghijklmnopqrstuvwxyz0123456789";
		$strchars="";
		$strlen=strlen($strtext);
		for($i=0;$i<$strnum;$i++){
			$strpos=rand(0,$strlen);
			$strchars.=substr($strtext,$strpos,1);
		}
		return $strchars;
	}
	...
	function CreateTempName(){
		...
		$strTime.=GetRandWords(4);
		return md5($strTime);
	}
	
	function GetTempName(){
		$strName= CreateTempName();
		$strTemp=substr($strName,0,8)."-";
		$strTemp.=substr($strName,8,4)."-";
		$strTemp.=substr($strName,12,4)."-";
		$strTemp.=substr($strName,16,4)."-";
		$strTemp.=substr($strName,20,12);
		setcookie('User-keys',$strTemp,time()+60*60*24*30,'/');
		return $strTemp;
	}