<?php
class PagelistAction extends CommonAction{
	
public function index(){
		$result=R('Table/Page/reList');
		$this->assign("PageList",$result['lists']);
	...
	public function saveadd(){
		...
		$strData=R("Table/Page/checkData",array($_POST,$dataType));
		if(!$strData){
	...
	public function edittable(){
		...
		if($lid){
			$result  =R("Table/Page/reContent",array($lid));
			if(is_array($result)){
	...
	public function dellist(){
		$pid=$_POST['id'];
		if(!empty($pid)){
			$reStatus=R("Table/Page/delData",array($pid));
